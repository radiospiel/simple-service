module Simple::Workflow
  module CurrentContext
    ...
    def context
      Thread.current[:"Simple::Workflow.context"] || raise(ContextMissingError)
    end
    ...
    def logger
      current_context = Thread.current[:"Simple::Workflow.context"]
    ...
    def with_context(ctx = nil, &block)
      old_ctx = Thread.current[:"Simple::Workflow.context"]

      if old_ctx && ctx.nil?
        block.call
      else
        begin
          Thread.current[:"Simple::Workflow.context"] = Context.new(ctx, old_ctx)

          block.call
        ensure
          Thread.current[:"Simple::Workflow.context"] = old_ctx
        end
      end
    end
  end
